...
func newGatherBootstrapCmd() *cobra.Command {
	cmd := &cobra.Command{
		...
		RunE:  runGatherBootstrapCmd,
	}
	cmd.PersistentFlags().StringVar(&gatherBootstrapOpts.bootstrap, "bootstrap", "", "")
	cmd.PersistentFlags().StringArrayVar(&gatherBootstrapOpts.masters, "master", []string{}, "")
	return cmd
}

func runGatherBootstrapCmd(_ *cobra.Command, _ []string) error {
	cleanup := setupFileHook(rootOpts.dir)
	defer cleanup()

	tfStateFilePath := filepath.Join(rootOpts.dir, terraform.StateFileName)
	...
	spew.Dump(bootstrap, masters)
	return nil
}

func logGatherBootstrap(boostrap string, masters []string) {
